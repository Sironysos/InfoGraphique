...
void initialize_scene( Viewer& viewer )
{
	...
	// Keyframe animation for the train
    train->addGlobalTransformKeyframe(getTranslationMatrix(230, 0, -4.5) * getRotationMatrix(M_PI * 0.5, glm::vec3(0, 1, 0))*getScaleMatrix(0.25,0.25,0.25), 0);
    train->addGlobalTransformKeyframe(getTranslationMatrix(-245, 0, -4.5) * getRotationMatrix(M_PI * 0.5, glm::vec3(0, 1, 0))*getScaleMatrix(0.25,0.25,0.25), 30);
    ...
    for (int i = 0; i < 2; i++) {
        ...
        rail->addGlobalTransformKeyframe(getRotationMatrix(M_PI*0.5,glm::vec3(0,1,0))*getScaleMatrix(0.5,0.5,0.5)*getTranslationMatrix(0, -1.2, 3.6*i-1), 0);
        rail->addGlobalTransformKeyframe(getRotationMatrix(M_PI*0.5,glm::vec3(0,1,0))*getScaleMatrix(0.5,0.5,0.5)*getTranslationMatrix(9, -1.2, 3.6*i-1), 10);
        rail->addGlobalTransformKeyframe(getRotationMatrix(M_PI*0.5,glm::vec3(0,1,0))*getScaleMatrix(0.5,0.5,0.5)*getTranslationMatrix(9, -1.2, 3.6*i-1), 30);
    ...
    TexturedLightedMeshRenderablePtr box = std::make_shared<TexturedLightedMeshRenderable>(texShader, box_path, materials2[0], box_texture_path);
    box->setGlobalTransform(glm::mat4(1.0f));

    viewer.addRenderable(box);

    // Keyframe animation for the box movement
    box->addGlobalTransformKeyframe(glm::vec3(0), 0);
    box->addGlobalTransformKeyframe(getTranslationMatrix(0, 0, -4.25), 10);
    box->addGlobalTransformKeyframe(getTranslationMatrix(0, 0, -4.25), 30);
    ...
    wall->addGlobalTransformKeyframe(getTranslationMatrix(240, 0, -4.5), 0);
    wall->addGlobalTransformKeyframe(getTranslationMatrix(-240, 0, -4.5), 30);
}
